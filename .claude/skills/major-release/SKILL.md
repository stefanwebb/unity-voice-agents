---
name: major-release
description: Use when releasing a new major version of the Voice Agents Unity package to GitHub
---
...
Read `package.json` for `"version": "X.Y.Z"`. Next version is `(X+1).0.0`.

### 2. Update version

Edit `package.json`: set `"version": "(X+1).0.0"`. This is the only place the version lives — there is no `pyproject.toml`, `.csproj`, or `AssemblyInfo` to keep in sync. Do **not** touch the `"unity"` field (minimum Editor version) unless the release genuinely raises it.
...
Replace the entire contents of `RELEASE.md` with only the body of the new changelog entry (the sections under the new version heading). **No top-level header** — start directly with `### Breaking changes` or equivalent. This file is the GitHub release notes.
...
```
https://github.com/stefanwebb/unity-voice-agents.git#v(X+1).0.0
```
